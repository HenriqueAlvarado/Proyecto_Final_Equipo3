...
// POST /api/pacientes — Recepcionista registra paciente con contraseña
router.post('/', async (req, res) => {
  ...
  if (!email) {
    return res.status(400).json({ message: 'El correo es requerido para crear acceso al paciente' });
  }
  ...
  try {
    ...
    const result = await pool.query(
      `INSERT INTO pacientes (nombre, apellido, email, password_hash, telefono, fecha_nacimiento, curp, direccion)
       VALUES ($1, $2, $3, $4, $5, $6, $7, $8) RETURNING id, nombre, apellido, email, telefono, fecha_nacimiento, curp`,
...
// GET /api/pacientes/:id/citas — Citas del paciente (usado por el paciente autenticado)
router.get('/:id/citas', async (req, res) => {
  // Solo el propio paciente o staff puede ver sus citas
